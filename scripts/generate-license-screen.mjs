...
const androidInventoryPath = join(root, 'docs/ANDROID-DEPENDENCIES.md');
...
// ---------------------------------------------------------------- the Gradle half
//
// `docs/ANDROID-DEPENDENCIES.md` is generated by `./gradlew :app:androidLicenceInventory`
// from the release runtime classpath: androidx, CameraX, ZXing, Kotlin, OkHttp. Those
// modules ship inside the APK without being linked into `libgojni.so`, and until this
// block existed their licence texts lived only in the repository, never in the binary.
//
// A POM names a licence, never a text, so the names are mapped here to the texts the
// app already carries. An unmapped name is a build error: a new dependency with a new
// licence has to be added deliberately rather than appear in the app unlicensed.
const ANDROID_LICENCES = new Map([
  ['The Apache Software License, Version 2.0', { id: 'Apache-2.0', text: 'Apache-2.0.txt' }],
  ['The Apache License, Version 2.0', { id: 'Apache-2.0', text: 'Apache-2.0.txt' }],
]);

const apacheSource = entries.find((entry) => entry.licence === 'Apache-2.0');
if (!apacheSource) {
  fail('licenses/ carries no Apache-2.0 text, which the Android dependencies are shipped under');
} else {
  // Shipped under a name that says what it is. The Go-side copies are named after the
  // Go module whose copyright line they carry; this is the same text under a plain name.
  bodies.set('Apache-2.0.txt', readFileSync(join(licencesDir, apacheSource.file)));
}

const androidRows = [];
if (!existsSync(androidInventoryPath)) {
  fail('docs/ANDROID-DEPENDENCIES.md is missing; run ./gradlew :app:androidLicenceInventory');
} else {
  for (const line of readFileSync(androidInventoryPath, 'utf8').split('\n')) {
    const row = line.match(/^\| `([^`]+)` \| ([^|]+) \| ([^|]+) \|$/);
    if (!row) continue;
    const module = row[1];
    const version = row[2].trim();
    const declared = row[3].trim();
    // A module may declare more than one licence: androidx.camera:camera-core declares
    // "The Apache Software License, Version 2.0; BSD License", the BSD half covering the
    // libyuv it bundles. A dual licence lets the distributor satisfy either one, so the
    // first declaration with a text the app carries is what ships — and the full
    // declaration stays in docs/ANDROID-DEPENDENCIES.md, so the choice is visible
    // rather than implied.
    const mapped = declared
      .split(';')
      .map((part) => ANDROID_LICENCES.get(part.trim()))
      .find(Boolean);
    if (!mapped) {
      fail(`docs/ANDROID-DEPENDENCIES.md declares a licence this script cannot ship: "${declared}" (${module})`);
      continue;
    }
    androidRows.push({
      key: `android:${module}`,
  ...
  if (androidRows.length === 0) fail('docs/ANDROID-DEPENDENCIES.md lists no modules');
  index.push(...androidRows);
}

const notes = [
  'This app ships a native library, `libgojni.so`, that statically links the Go',
  'modules listed first here, and it ships the Android and Kotlin modules listed after',
  'them. BSD-3-Clause and Apache-2.0 attach conditions to binary distribution: the',
  'copyright notice, the list of conditions, the disclaimer, and any upstream NOTICE, so',
  'the texts travel inside the app rather than only in the repository. The same files are',
  'published at THIRD-PARTY-NOTICES.md and docs/ANDROID-DEPENDENCIES.md.',
  '',
  `Scope: ${entries.length} licence texts covering the Go modules linked into the native`,
  `bridge, then ${androidRows.length} Android and Kotlin modules taken from the Gradle`,
  'graph, each with the licence its own POM declares. Both halves are generated from',
  'those sources; neither list is typed by hand.',
];
