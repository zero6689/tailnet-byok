...
This project handles credentials, and security reports are inherently sensitive:

- **Do not disclose someone else's credential**, even incidentally. If a user pastes a real auth
  key into an issue, do not quote it back; ask them to rotate it, and report the comment for
  removal.
- **Do not publish a working exploit** for an unfixed vulnerability, and do not pressure anyone
  to disclose one before a fix ships. See [SECURITY.md](SECURITY.md) for the coordinated process.
- **Critique the design, not the person.** "This approach has a re-identification problem
  because…" is welcome. "This is obviously wrong" is not, and it is also less useful.
...
community leaders responsible for enforcement by opening a confidential report through GitHub's
[report abuse](https://github.com/contact/report-abuse) flow, or by contacting the maintainer
